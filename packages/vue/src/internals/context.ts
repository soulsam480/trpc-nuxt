...
import { AnyRouter, inferHandlerInput, inferProcedureInput, inferProcedureOutput } from '@trpc/server'
...
export interface TRPCContextState<TRouter extends AnyRouter, TSSRContext = undefined> {
  ...
  >(
    ...
    opts?: MaybeRef<TRPCFetchQueryOptions<TInput, TRPCClientError<TRouter>, TOutput>>,
  ): Promise<TOutput>
  /**
   * @link https://react-query.tanstack.com/guides/prefetching
   */
  ...
  >(
    pathAndInput: [path: TPath, ...args: inferHandlerInput<TProcedure>],
    opts?: MaybeRef<TRPCFetchInfiniteQueryOptions<TInput, TRPCClientError<TRouter>, TOutput>>,
...
export const TRPCContextKey: InjectionKey<any> = __DEV__ ? Symbol('nuxt-trpc') : Symbol()
